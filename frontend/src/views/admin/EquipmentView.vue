...
function parseCsv(text: string): PreviewRow[] | null {
  ...
  let startIndex = 0
  if (lines.length > 0 && /^(building|room|equipment)/i.test(lines[0]!.trim())) {
    startIndex = 1
  }
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    if (!building || !roomNumber || !name) continue
    rows.push({ building, roomNumber, name, roomId: null })
  }

  if (rows.length === 0) {
    fileError.value = 'No valid rows found. Expected: building, room, equipment'
    return null
  }
...
function onFileChange(files: File[] | File | null) {
  ...
  reader.onload = (e) => {
    ...
    const existingSet = new Set(
      equipment.value.map(eq => `${eq.roomId}|${eq.name.toLowerCase()}`)
    )
    ...
    for (const row of rows) {
      ...
      } else if (existingSet.has(`${roomId}|${row.name.toLowerCase()}`)) {
...
async function confirmImport() {
  ...
  try {
    const items = previewToImport.value.map(row => ({
      roomId: row.roomId!,
      name: row.name,
    }))
    const result = await bulkImportEquipment(items)
    csvDialog.value = false
    snackbarText.value = `Imported ${result.imported} item${result.imported !== 1 ? 's' : ''}${result.skipped ? ` (${result.skipped} skipped)` : ''}.`
    snackbar.value = true
    await load()
  } finally {
...
<template>
  <AppLayout>
    ...
    <!-- CSV import dialog -->
    <v-dialog v-model="csvDialog" max-width="780" scrollable>
      <v-card title="Import Equipment from CSV">
        <v-card-text>
          <v-alert type="info" density="compact" variant="tonal" class="mb-4">
            3 columns: <strong>building, room, equipment</strong> — e.g. <em>Livingstone Tower, LT101, Projector</em>
          </v-alert>
